...
export function LiveBoard({visual, phase, rowCount}: Props) {
    ...
    if (visual.type === "throws") {
        return (
            <div
                className={live ? "stamp-in rounded-lg border border-destructive/40 bg-destructive/10 p-4" : "rounded-lg border border-dashed border-border p-4"}>
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                    <Chip label="a" value={visual.a} delay="0ms" show={live}
                          dim={Boolean(visual.focus && visual.focus !== "a")}/>
                    <Chip label="op" value={visual.op} delay="70ms" show={live}
                          dim={Boolean(visual.focus && visual.focus !== "op")}/>
                    <Chip label="b" value={visual.b} delay="140ms" show={live}
                          dim={Boolean(visual.focus && visual.focus !== "b")}/>
                    <Chip label="c" value={visual.c} delay="210ms" show={live}
                          dim={Boolean(visual.focus && visual.focus !== "c")}/>
                </div>
                {visual.focus ? (
                    <p className="text-sm text-muted-foreground">Este test solo afirma ese dato. El resto se ve apagado
                        a propósito.</p>
                ) : (
                    <p className="text-sm text-muted-foreground">Todavía no hay x. Eso es el test unitario del
                        parseador.</p>
                )}
            </div>
        );
